...
const string RedPiece::m_Name = "Red";
const ofColor RedPiece::m_MainColor = ofColor(255, 30, 10);
const ofColor RedPiece::m_SecondaryColor = ofColor(160, 30, 10);
int RedPiece::m_PiecesOnBoard = 0;
...
RedPiece::RedPiece(const float &a_X, const float &a_Y, const float &a_R) {
  setPosition(a_X, a_Y);
  setRadius(a_R);
  m_PiecesOnBoard++;
...
RedPiece::RedPiece(const RedPiece &original) {
...
void RedPiece::setPosition(const float &x, const float &y) {
  (x > -getRadius()) ? setX(x) : setX(getDiameter());
  (y > -getRadius()) ? setY(y) : setY(getDiameter());
}

void RedPiece::setRadius(const float &r) {
  m_Radius = (r > 0) ? r : 20;
}
...
void RedPiece::resize(const float &x, const float &y, const float &r) {
...
void RedPiece::drop(const float &start, const float &end, const float &rate) {
  m_Dropping = true;
  show();
  dropDown(start, end, rate);
}

bool RedPiece::dropDown(const float &yStart, const float &yEnd, const float &rate) {
